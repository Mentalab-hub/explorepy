...
vector<device> ExploreSDK::PerformDeviceSearch(int length)
{
  ...
  (void)(length);
	...
	if (querySet == nullptr)
		throw ExploreException("Out of memory: Unable to allocate memory resource for inquiry");
	...
	if (lookupServiceError == SOCKET_ERROR)
	{
		if (errorCode == WSASERVICE_NOT_FOUND) {
			...
			throw ExploreException(BluetoothHelpers::GetWSAErrorMessage(errorCode));
		}
	}
...
int ExploreSDK::SdpSearch(string address)
{
	// Construct windows socket bluetooth variables
	...
	if (querySet == nullptr)
		throw ExploreException("Out of memory: Unable to allocate memory resource for sdp search");
	...
	if (lookupServiceError == SOCKET_ERROR)
		throw ExploreException(BluetoothHelpers::GetWSAErrorMessage(WSAGetLastError()));
