...
def main():
    ...
    exp_device.connect(device_name='Explore_AAAQ')
    time.sleep(1)
    exp_device.start_asr_calib()
    time.sleep(2)
    # Subscribe your function to the stream publisher
    exp_device.stream_processor.subscribe(callback=my_exg_function, topic=TOPICS.asr_ExG)
    try:
        while True:
            time.sleep(.5)
